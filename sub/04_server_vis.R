...
output$download_carnival <- downloadHandler(
  filename = "carnivalRes.sif",
  content = function(con) {
    write.csv(values$carnival_result$weightedSIF, con,quote=F,row.names = F)
  }
)

# enable downloader
observe({
  res = values$carnival_result
  if(is.null(res)){
    disable("download_carnival")
  }else{
    enable("download_carnival")
  }
})
...
generateNetwork = function(){
  ...
  values$carnival_result = carnival_result
